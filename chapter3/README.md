...
如同大多數的作業系統，xv6 使用分頁硬體來進行記憶體保護與映射。 大多數作業系統會比 xv6 更複雜地使用分頁技術，透過分頁與頁面錯誤例外的結合來達成，這部分我們會在第四章內討論

xv6 簡化了實作，因為核心直接使用虛擬位址與實體位址的一對一映射，並假設物理 RAM 位於 `0x80000000` 這個位址，同時也是核心預期載入的位置。 這在使用 QEMU 時可以正常運作，但在真實硬體上卻是個壞主意，因為真實硬體的 RAM 與裝置會被配置在不可預測的物理位址上，例如在某些系統中，`0x80000000` 處可能根本沒有 RAM，而這正是 xv6 預期存放核心的位置。 更嚴謹的核心設計會利用頁表將任意的物理記憶體配置映射成可預測的核心虛擬位址佈局

RISC-V 支援針對實體位址層級的保護功能，但 xv6 並未使用這項功能

在擁有大量記憶體的機器上，使用 RISC-V 所支援的「超大頁面（super pages）」是合理的。 但當實體記憶體很小時，使用小頁面比較合理，這樣可以以更細緻的粒度進行配置與換頁（page-out）到硬碟。 例如，如果一個程式只用到 8 KB 記憶體，卻給它一整個 4 MB 的超大頁面，那就很浪費。 在具備大量 RAM 的機器上，使用大頁面比較合理，並且可以減少管理頁表的負擔
...
2. 撰寫一個使用者程式，透過呼叫 `sbrk(1)` 以增加一個位元組的位址空間。 執行程式，並觀察呼叫 `sbrk` 前後該程式的頁表。 核心實際上分配了多少空間？ 新分配的記憶體對應的 PTE 中包含了哪些內容？
3. 修改 xv6，使核心可以使用超大頁面（super pages）
